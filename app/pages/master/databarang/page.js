...
export default function Home() {
  const { axiosJWT, token, jenis, databarang, getData_Barang, col, MySwal, toat } = useContext(Contex);
  ...
  return (
    <div className={`mx-15 text-slate-500 max-h-full min-h-screen pb-10`}>
          ...
          <table className="border-collapse table-auto text-sm w-11/12 mx-auto text-center">
            <thead className="blue2 text-slate-100">
              <tr>
                ...
                <th className={`border border-gray-300 font-medium p-4 pl-8 pb-3 rounded-t-lg  ${!chkbox2 ? "hidden" : ""}`}>Satuan</th>
                <th className={`border border-gray-300 font-medium p-4 pl-8 pb-3 rounded-t-lg  ${!chkbox3 ? "hidden" : ""}`}>Bahan</th>
                <th className={`border border-gray-300 font-medium p-4 pl-8 pb-3 rounded-t-lg  ${!chkbox4 ? "hidden" : ""}`}>Stock</th>
                <th className={`border border-gray-300 font-medium p-4 pl-8 pb-3 rounded-t-lg  ${!chkbox5 ? "hidden" : ""}`}>Action</th>
              </tr>
            </thead>
            <tbody>
              {databarang?.map((item) => (
                <tr key={item.id}>
                  ...
                  <td className={`border border-gray-300 p-4 pl-8 text-slate-500  ${!chkbox2 ? "hidden" : ""}`}>
                    <select className={`w-full p-2 my-5 border border-gray-300 rounded-lg ${disableds == item.id ? "" : "hidden"}`} onChange={(e) => setJenisvalue(e.target.value)} fdprocessedid="true" value={jenisvalue || ""}>
                      {jenis?.map((item) => (
                  ...
                  <td className={`border border-gray-300 p-4 pl-8 text-slate-500  ${!chkbox3 ? "hidden" : ""}`}>
                    <input
                      value={bahan || ""}
                    ...
                    />
                    <p className={`${disableds == item.id ? "hidden" : ""}`}>{item.bahan.replace("[", "").replace("]", "")}</p>
                  </td>
                  <td className={`border border-gray-300 p-4 pl-8 text-slate-500  ${!chkbox4 ? "hidden" : ""}`}>
                  ...
                  <td className={`border border-gray-300 p-4 pl-8 text-slate-500 ${!chkbox5 ? "hidden" : ""}`}>
                    <div className="basis-1/12 flex flex-row text-center" data-key={item.id}>
                      ...
                      <p className={`basis-1/2 cursor-pointer ${disableds == item.id ? "hidden" : ""}`} data-key={item.id} data-key1={item.nama} data-key2={item.jenis} data-key3={item.stok} onClick={updateinput}>
                        <FontAwesomeIcon icon={faPencil} data-key={item.id} data-key1={item.nama} data-key2={item.jenis} data-key3={item.stok} onClick={updateinput} />
                      </p>
      ...
      <ul className={`max-[800px]:top-[380px] h-fit blue text-slate-100 w-fit absolute top-[340px] postionus rounded-lg ${col == 0 ? "hidden" : ""}`}>
        ...
        <li className="w-30 py-5 pb-3 px-5 flex">
          <input type="checkbox" defaultChecked value={3} onClick={(e) => setChkbox4(e.target.checked)} />
