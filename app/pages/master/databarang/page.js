...
export default function Home() {
  const { axiosJWT, token, jenis, databarang, getData_Barang, col, MySwal, setBahanvalue, setData, router } = useContext(Contex);
                ...
                <tr key={item.id}>
                  ...
                  <td className={`border border-gray-300 p-4 pl-8 text-slate-500 ${!chkbox5 ? "hidden" : ""}`}>
                    <div className="basis-1/12 flex flex-row text-center">
                      <p className={`basis-1/2 cursor-pointer text-center`} data-key2={item.bahan} data-key={[item.id, item.nama, item.jenis, item.stok, item.bahan]} onClick={updateinput}>
                        <svg
                          className="m-auto"
                          data-key={[item.id, item.nama, item.jenis, item.stok, item.bahan]}
                        ...
                        >
                          <path
                            data-key={[item.id, item.nama, item.jenis, item.stok]}
                            data-key2={item.bahan}
                            onClick={updateinput}
